...
def simulation(orbitingBody, centralBody, ax, fig):
    ...
    acumulated_positions = []
    scatter = ax.scatter([], [], color="Red", s=2)
    
    camera = Camera(fig)
    for i, point in enumerate(x_positions):
        acumulated_positions.append([x_positions[i], y_positions[i]])
        scatter.set_offsets(acumulated_positions)
        plt.pause(0.1)
        camera.snap()
    
    animation = camera.animate()
    animation.save("animation.gif", writer="Pillow", fps=120)